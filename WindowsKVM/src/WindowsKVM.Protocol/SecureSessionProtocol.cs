...
{
    /// Version 2 includes the authenticated heartbeat used by MacKVM to
    /// expire a session whose TCP socket remains open while a peer is asleep
    /// or stalled. Windows advertises and validates the same capability so a
    /// current Mac can establish a bounded-liveness session.
    public const int CurrentDisconnectSignalVersion = 2;
...
public static class SecureSessionControlSignal
{
    ...
    private static readonly byte[] HeartbeatBytes =
        Encoding.UTF8.GetBytes("MacKVM secure session heartbeat v1");
    private static readonly byte[] HeartbeatAcknowledgementBytes =
        Encoding.UTF8.GetBytes(
            "MacKVM secure session heartbeat acknowledgement v1"
        );
    ...
    public static ReadOnlyMemory<byte> Heartbeat => HeartbeatBytes;
    public static ReadOnlyMemory<byte> HeartbeatAcknowledgement
        => HeartbeatAcknowledgementBytes;
    ...
    public static bool IsHeartbeat(ReadOnlySpan<byte> payload)
        => payload.SequenceEqual(HeartbeatBytes);

    public static bool IsHeartbeatAcknowledgement(ReadOnlySpan<byte> payload)
        => payload.SequenceEqual(HeartbeatAcknowledgementBytes);
}
