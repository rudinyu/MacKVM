...
internal static class Program
{
    ...
    private static void TestSecureSessionHandshakeRoundTrip()
    {
        ...
        Assert(
            json.Contains("\"disconnectSignalVersion\":2", StringComparison.Ordinal),
    ...
    private static void TestSecureSessionDisconnectSignalsRequireExactPayload()
    {
        ...
        Assert(
            SecureSessionControlSignal.IsHeartbeat(
                SecureSessionControlSignal.Heartbeat.Span
            ),
            "heartbeat signal should match exactly"
        );
        Assert(
            SecureSessionControlSignal.IsHeartbeatAcknowledgement(
                SecureSessionControlSignal.HeartbeatAcknowledgement.Span
            ),
            "heartbeat acknowledgement should match exactly"
        );
        Assert(
            !SecureSessionControlSignal.IsHeartbeat(
                SecureSessionControlSignal.HeartbeatAcknowledgement.Span
            ),
            "heartbeat acknowledgement must not be treated as a request"
        );
    }
