...
本路線圖以 1.100.01 程式碼為基準。配對、公開金鑰釘選、加密連線、輸入驗證、
...
| 領域 | 目前行為 |
| --- | --- |
| 輸入轉送 | 鍵盤（含跨配置重映射）、滑鼠、觸控板指標／點按／拖曳、高解析度雙軸捲動（含 line／pixel 單位、相位／慣性）、硬體暴露出的 pressure 與白名單媒體鍵；剪貼簿仍待補上 |
...
- **F5 觸控板指標與捲動完整性**：`RemoteInputEvent` 支援觸控板相容的指標壓力、
  高解析度 fixed-point 位移、line／pixel 單位、連續捲動，以及可選的 `scrollPhase`／
  `scrollMomentumPhase`。缺欄位代表「無相位」，滑鼠滾輪與舊版 peer 仍相容；觸控板的
  點按、次要點按、拖曳、精準雙軸捲動、相位與慣性都沿用驗證後的加密輸入路徑。
...
高階觸控板手勢（Pinch、旋轉、三指工作區滑動）無法透過公開 `CGEvent` 全域注入，可能
需要受限或私有 API；檔案傳輸會擴大攻擊面並需要分片與背壓。休眠／喚醒後的安全重連
已完成，但喚醒已關機或完全不可達的 Mac 不在 app 範圍內。其餘研究項目應先完成可行性
研究再排期。

## 建議順序

F3、F5、F6、F7、F13 都已完成。下一步是 **F1**：範圍獨立、不動控制狀態機，而且
