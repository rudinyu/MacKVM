...
- 驗證後的鍵盤、滑鼠與觸控板轉送，包括高解析度雙軸捲動、捲動相位／慣性、點按／拖曳／
  次要點按狀態，以及 macOS 暴露時的 pressure；並搭配明確的 Allow／Deny 控制同意。
...
可注入完整的鍵盤、滑鼠，以及目前公開 `CGEvent` 能表達的觸控板指標／捲動事件。公開 API
可以保留觸控板移動、點按、拖曳、精準雙軸捲動、慣性相位與暴露出的 pressure；pinch、旋轉、
三指工作區滑動等 AppKit-only 手勢無法透過公開 `CGEvent` 全域注入，因此不列入本版本驗收。
