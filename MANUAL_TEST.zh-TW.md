...
8. 在配置不同的狀態下，打一個接收端配置沒有對應字元的按鍵，確認遠端輸入安全
   停止（清楚的狀態訊息、沒有卡住的按鍵或滑鼠按鈕），之後重新請求控制能正常
   成功。測試後把兩台恢復成相同鍵盤配置。
9. 控制中按住控制端實體鍵盤的音量鍵與亮度鍵，確認只有接收端的音量／亮度改變，
   控制端本身不受影響。
10. 控制中按控制端實體鍵盤的電源鍵，確認兩台都沒有任何反應（接收端不會跳出關機
    或睡眠對話框）；按 Caps Lock 確認只切換一次、不會誤觸發兩次。
11. 用 **Return input to this Mac**、接收端 **Stop remote control** 及
