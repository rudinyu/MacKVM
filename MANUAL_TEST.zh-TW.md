...
7. 在配置不同的狀態下，測試需要重映射按鍵的 Cmd 快捷鍵（例如 Cmd-C 複製），確認
   快捷鍵動作正確觸發，而不是把按鍵當文字打出來。
8. 在配置不同的狀態下，把控制端的 Caps Lock 打開，重複一次步驟 7 的 Cmd 快捷鍵。
...
步驟 7／8 的 Cmd 快捷鍵不論 Caps Lock 開關都觸發同一個動作（不會變成
Cmd-Shift-C 之類的錯誤快捷鍵）——Cmd／Control 按鍵完全跳過重映射，照原始
keyCode／flags 注入；步驟 9 放開字母鍵後不會留下卡住或持續連發的字元，連發中途
