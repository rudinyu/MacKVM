...
本手冊說明如何建置 MacKVM 的 Windows 分支。目前 W1 內容是 C#/.NET 8 protocol
library、簽章配對 state machine、console 配對 receiver 與簡易 mDNS 廣播器；它可以
與 MacKVM 1.00.00 建立簽章信任關係。這還不是完整 Windows KVM；WinUI 3、Raw Input、
SendInput、加密控制 session、快捷鍵與 tray integration 會在後續 W2 實作。
...
在 Windows publish 後啟動 W1 receiver：

```powershell
.\dist\windows\arm64\WindowsKVM.exe --pairing-listen --name "Windows ARM64"
```

正常流程會顯示六位數驗證碼；請把它與發起配對的 Mac 畫面比對，再在 Windows
輸入 `y`：
...
只有在受控測試時才使用 `--yes`，它會自動接受驗證碼。receiver 會在區域網路廣播
`_mackvm._tcp`，並預設監聽系統分配的 TCP port。Windows Defender Firewall 可能顯示
標準的 Private network 提示；只在信任的區域網路允許此程式。程式不會偷偷新增寬鬆的
防火牆規則。
...
DPAPI 保護，絕對不要把 `identity.json` 複製到另一台電腦。W1 console 目前只完成配對
與信任建立；鍵盤／滑鼠控制尚未啟用。
...
腳本本身已會拒絕錯誤的 PE machine type。W1 receiver 可以完成與 macOS 的簽章配對，
但尚未提供完整的 Windows 輸入控制功能。
...
Windows executable 尚未加入 macOS DMG packaging。W1 只驗證跨平台簽章配對；加密控制
session、鍵盤／滑鼠接管與交還會在 W2 完成後，才會宣告 Windows 版可用。
